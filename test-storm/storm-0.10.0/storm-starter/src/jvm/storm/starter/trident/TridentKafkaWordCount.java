 ...
 * tutorial</a>, <a
 * href="https://storm.apache.org/documentation/Trident-state">trident state</a>
 * and <a
 * href="https://github.com/apache/storm/tree/master/external/storm-kafka">
 * Storm Kafka </a>.
 * </p>
 */
public class TridentKafkaWordCount {
    ...
    /**
     * Creates a transactional kafka spout that consumes any new data published
     * to "test" topic.
     * <p/>
     * For more info on transactional spouts see "Transactional spouts" section
     * in <a href="https://storm.apache.org/documentation/Trident-state">
     * Trident state</a> doc.
     *
    ...
    private Stream addDRPCStream(TridentTopology tridentTopology, TridentState state, LocalDRPC drpc) {
        return tridentTopology.newDRPCStream("words", drpc).each(new Fields("args"), new Split(), new Fields("word"))
                .groupBy(new Fields("word")).stateQuery(state, new Fields("word"), new MapGet(), new Fields("count"))
                .each(new Fields("count"), new FilterNull()).project(new Fields("word", "count"));
    }
    ...
    public StormTopology buildProducerTopology() {
         ...
         * the kafka topic.
         */
        KafkaBolt bolt = new KafkaBolt().withTopicSelector(new DefaultTopicSelector("test")).withTupleToKafkaMapper(
