defmodule MyApp.CLI do
  ...
  def fentan1(items, reduce_total) do
    items =
      Enum.map(items, fn {id, price, amount} ->
  ...
  defp _fentan1([{id, _, _}], _total, reduce_total, acc, ret),
    do: [{id, reduce_total - acc} | ret]
  ...
  # v |> (fn x -> x * 100.0 end).() |> Float.round |> round
  defp to_cent_round(v), do: round(v * 100.0)
