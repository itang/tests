...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println();
        System.out.println(new SqlVisitor().visit(f5));
    }
    ...
    public static class SqlVisitor implements Visitor<String> {
        ...
        @Override
        public String visitAnd(And it) {
            return it.getItems().stream().map(_it -> "(" + vistor(_it) + ")").collect(Collectors.joining(" and "));
        }

        @Override
        public String visitOr(Or it) {
            return it.getItems().stream().map(_it -> "(" + vistor(_it) + ")").collect(Collectors.joining(" or "));
        }
    }
    ...
    public static abstract class Comp implements Filter {
